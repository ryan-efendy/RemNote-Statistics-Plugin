...
import HeatMap2 from '@uiw/react-heat-map';
...
const LIMIT = 1672560000000; // 01-01-2023
...
export const Heatmap = () =>
{
  ...
  // deep copy repetitionsPerDay
  const repetitionsPerDayCopy = JSON.parse( JSON.stringify( repetitionsPerDay ) );
  var fullArrayRepetitionsPerDay = getFullArrayRepetitionsPerDay(repetitionsPerDayCopy);
  // debugger;
...
function getRepetitionsPerDayObject ( allCards )
{
  ...
  const repetitionHistoryDatesFlatSortedDatesGroupedByDayUnix = Object.keys( repetitionHistoryDatesFlatSortedDatesGroupedByDay || {} ).map( ( key ) =>
  {
    return {
      ...
      count: repetitionHistoryDatesFlatSortedDatesGroupedByDay[ key ]
    }
  } );
